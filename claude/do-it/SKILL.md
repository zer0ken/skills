...
하네스는 하나의 세션이 컨트롤러가 되어, 코딩 작업을 유형별로 다른 모델의 에이전트에게 위임하고, tmux 윈도우에서 각 에이전트의 CLI를 실행하며, 계획-실행-검사-병합 루프를 완료될 때까지 돌리는 작업 방식이다.

컨트롤러는 이 스킬을 호출한 세션이다. 컨트롤러는 claude, codex, pi 중 무엇이어도 된다. 컨트롤러는 tmux 세션의 서로 다른 윈도우에서 claude code cli, codex cli, pi cli를 실행한다. 각 에이전트는 임무 파일(mission)을 읽고, 산출물을 결과 파일(result)에 쓰고, 완료 마커(done)를 남긴다. 컨트롤러는 파일로 상태를 추적하고 루프를 결정한다. 컨트롤러가 tmux 윈도우에 직접 키 입력을 보내지 않는다.
...
작업 판단은 컨트롤러가 한다. 작업이 작으면 이 스킬을 적용하지 않고 곧바로 처리한다.
...
| --- | --- | --- | --- | --- |
| 설계자(계획) | codex | gpt-6-astra | medium | 계획 작성, diff 리뷰, 루프 결정 |
...
구현자와 검사자는 작업 단위마다 한 쌍씩 생성한다. 검사자는 구현자 세션 하나당 하나를 생성한다. 설계자는 계획 작성과 계획 리뷰가 모델이 다르다. 병합자는 설계자와 같은 수준의 모델을 쓰는 별개 세션이다.

### 모델 선택 규칙

저수준(low-level) 지식이 많이 필요한 작업은 codex를 claude보다 선호한다. 컨트롤러는 시작 절차에서 작업 특성을 판단하고, 계획 리뷰에 쓸 모델을 정한다.
...
이 신호가 충분하지 않으면 claude를 쓴다. 판단은 컨트롤러가 brief를 보고 하며, 결과와 근거를 state.log에 기록한다.
...
각 실행(run)은 실행 디렉터리에 상태를 둔다. 에이전트와 컨트롤러는 파일로만 소통한다.

```
~/.do-it-runs/<run-id>/
  brief.md               # 작업 요약
  state.log              # 컨트롤러의 상태 기록 (보고 자료의 원천)
...
- 컨트롤러가 `missions/<역할>.md`를 쓰고 `results/<역할>.done`을 지운다.
- launch-role.ps1이 역할 윈도우를 띄우고, run-role.ps1이 임무를 읽어 에이전트를 실행한다.
- 에이전트가 종료하면 run-role.ps1이 `results/<역할>.done`에 종료 코드를 쓰고 끝난다.
- 컨트롤러가 wait-role.ps1로 완료 마커를 기다린다.
- 컨트롤러가 `results/<역할>.md`(산출물)와 `.done`(종료 코드)을 읽어 다음 단계를 결정한다.
- 컨트롤러가 산출물의 '범위 밖 발견' 항목을 `followups.md`에 모은다.

### 상태 기록

컨트롤러는 단계가 바뀔 때마다 state.log에 한 줄씩 추가한다. 최종 보고는 이 기록을 바탕으로 작성한다. 기록 형식은 다음과 같다.
...
2. 작업 특성 판단. brief를 보고 저수준 지식 필요도를 판단해 계획 리뷰에 쓸 모델을 정한다. 모델 선택 규칙을 따른다. 판단 결과와 근거를 state.log에 기록한다.
3. 저장소 확인. `git rev-parse --show-toplevel`로 저장소 경로와 base 브랜치를 정한다. base는 사용자가 지정하지 않으면 현재 브랜치로 한다.
4. 실행 디렉터리 생성. `~/.do-it-runs/<run-id>/`에 missions, results, logs, wt, reports 하위 디렉터리를 만든다. run-id는 `<슬러그>-YYYYMMDD-HHMM` 형식으로 한다.
5. tmux 세션 생성. 세션 이름은 `do-it-<슬러그>`로 하고 `tmux set -g remain-on-exit on`을 적용한다.
6. CLI와 모델 검증. `codex --version`, `claude --version`, `pi --list-models deepseek`로 실제 값을 확인하고 역할과 모델 표를 갱신한다.
7. state.log에 시작을 기록한다.
...
컨트롤러는 실행 디렉터리와 state.log를 읽고 `reports/report.md`를 작성한다. 보고 항목은 최종 보고 절을 따른다. 보고를 마치면 사용자에게 결과를 안내하고, 사용자가 확인할 수 있게 tmux 세션을 남겨 둔다.
...
- 전역 규칙(CLAUDE.md)과 프로젝트 규칙을 따른다.
...
템플릿의 `<...>` 자리를 컨트롤러가 채운다. 임무 파일은 에이전트가 직접 읽을 수 있는 완결된 지시여야 한다.
...
- 경로: <저장소 경로>
- 기준 브랜치: <base>
- 참고 문서: 전역 규칙(CLAUDE.md), 프로젝트 규칙, 기존 코드베이스 관행
...
| --- | --- |
| -Session | tmux 세션 이름 |
| -Role | 역할 이름 (designer, reviewer, impl-1, insp-1, diff, merger) |
...
역할 윈도우 안에서 실행되는 에이전트 러너다. launch-role.ps1이 인자로 넘긴다. 임무 파일을 읽어 CLI를 실행하고, 산출물 파일과 완료 마커를 쓴다. 컨트롤러가 직접 호출하지 않는다.

### wait-role.ps1

완료 마커를 기다린다. 인자는 -Done(마커 경로), -TimeoutSec(기본 120), -PollSec(기본 5)다. 마커가 나타나면 종료 코드를 출력하고, 시간 초과면 TIMEOUT을 출력하고 코드 1로 끝난다. 컨트롤러는 120초 청크로 반복 호출하고, 청크 사이에 로그 꼬리를 읽어 진행을 파악한다.
...
- 임시 판단. 컨트롤러가 스스로 판단하기 어려운 분기(작업 방향, 리스크 허용)는 사용자에게 결정을 요청한다.

## 최종 보고

컨트롤러는 실행 디렉터리와 state.log를 읽고 `reports/report.md`를 작성한다. 보고 항목은 다음과 같다.
...
- 완료 마커는 있는데 산출물이 없는 경우. 컨트롤러는 판정을 읽기 전에 산출물 파일 존재를 확인한다.
