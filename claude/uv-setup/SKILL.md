...
3. **Migration (poetry / pipenv / pip / pip-tools)** — use the unified tool:
   ...
   - **`--keep-current-data` is required**: without this flag it **auto-deletes the originals** — `requirements.txt`/`Pipfile`/poetry blocks (a violation of the CLAUDE.md "do not mutate originals" principle). Always pass the flag to keep the originals.
...
6. **Preserve originals after migration** — the `--keep-current-data` flag above keeps `requirements.txt`/`Pipfile`/poetry blocks (the CLAUDE.md "do not mutate originals" principle). Only advise the user to "remove them yourself after verifying the migration".
