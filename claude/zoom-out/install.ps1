#!/usr/bin/env pwsh
# Install or update the `zoom-out` skill into the personal claude skills dir.
# Re-running this fetches the latest version (install and update are identical).
$ErrorActionPreference = 'Stop'

$dest = Join-Path $env:USERPROFILE '.claude\skills\zoom-out'
$src  = 'https://raw.githubusercontent.com/zer0ken/skills/main/claude/zoom-out/SKILL.md'
...
Write-Host "zoom-out skill installed/updated at $dest"
Write-Host "Restart the agent (or open a new session) to pick it up, then run /zoom-out"