#!/usr/bin/env pwsh
# Install or update the `pr` skill into the personal claude skills dir.
# Re-running this fetches the latest version (install and update are identical).
$ErrorActionPreference = 'Stop'

$dest = Join-Path $env:USERPROFILE '.claude\skills\pr'
$src  = 'https://raw.githubusercontent.com/zer0ken/skills/main/claude/pr/SKILL.md'
...
Write-Host "pr skill installed/updated at $dest"
Write-Host "Restart the agent (or open a new session) to pick it up, then run /pr"