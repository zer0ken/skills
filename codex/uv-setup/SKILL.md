...
3. **마이그레이션 (poetry / pipenv / pip / pip-tools)** — 통합 도구 사용:
   ...
   - **`--keep-current-data` 필수**: 이 플래그가 없으면 `requirements.txt`/`Pipfile`/poetry 블록 등 **원본을 자동 삭제**한다(AGENTS.md "원본 비변형" 원칙 위반). 반드시 플래그를 붙여 원본을 남긴다.
...
6. **마이그레이션 후 원본 보존** — 위 `--keep-current-data` 플래그로 `requirements.txt`/`Pipfile`/poetry 블록 등이 보존된다(AGENTS.md "원본 비변형" 원칙). 사용자에게 "마이그레이션 검증 후 직접 제거하라"고 안내만 한다.
