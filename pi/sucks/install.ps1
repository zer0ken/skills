#!/usr/bin/env pwsh
# Install or update the `sucks` skill into the personal pi skills dir (~/.agents/skills).
# Re-running this fetches the latest version (install and update are identical).
$ErrorActionPreference = 'Stop'

$dest = Join-Path $env:USERPROFILE '.agents\skills\sucks'
$src  = 'https://raw.githubusercontent.com/zer0ken/skills/main/pi/sucks/SKILL.md'
...
Write-Host "Restart the agent (or open a new session) to pick it up."
